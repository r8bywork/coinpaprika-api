...
const Chart = ({selectedCoinId}:ChartProps) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const result = await axios.get(`http://localhost:5000/coin/chart/${selectedCoinId || 'btc-bitcoin'}?start=${'2023-01-01'}&interval=${selectedTimeframe}`);
            const chartData = Object.values(result.data).map((entry) => {
                return {
                    time: new Date(entry.timestamp).getTime() / 1000,
                    value: entry.price
                };
            });
            seriesRef.current && seriesRef.current.setData(chartData);
        };
        fetchData();
